...
from utils.data import 对比两个字典中的数据
...
def 后置操作_跳转回商铺走访页面(夹具_测试走访明细页面):
    yield 夹具_测试走访明细页面
    # 重新回到走访明细
...
class TestVisitDetailsBasic(BaseCase):
    ...
    @pytest.mark.usefixtures("后置操作_跳转回商铺走访页面")
    @pytest.mark.parametrize(
        "行号",
        [
            (1),
    ...
    def test_detail(self, 夹具_测试走访明细页面, 行号):
        ...
        商铺信息页面.输入查询条件(商铺名称=list_某行[2])
        商铺信息页面.click_button("搜索")
        商铺信息页面.等待表格加载完成()
        # # 查找表格中第 3 列（索引从 0 开始）值为 "目标内容" 的所有行
        loc_待查找的行 = 商铺信息页面.loc_表格中每列内容完全等于筛选条件的行({"名称": list_某行[2]})
        商铺信息页面.点击表格中某行按钮(loc_行=loc_待查找的行, 按钮名="详情")
        数据字典_商铺信息详情页 = 商铺信息页面.获取详情页中的数据(
            loc_表单项最上层=商铺信息页面.page.locator(".el-drawer").locator("visible=true"))
        商铺信息页面.关闭抽屉()
        映射关系 = {"商铺名称": "商铺名称", "商铺类型": "商铺类型", "所属社区": "所属居委会", "所属网格": "所属网格",
                    "商铺地址": "具体位置", "负责人": "负责人", "手机号码": "联系方式", "归属部门": "归属部门"}
        assert 对比两个字典中的数据(数据字典_走访明细详情页, 数据字典_商铺信息详情页, 映射关系)
