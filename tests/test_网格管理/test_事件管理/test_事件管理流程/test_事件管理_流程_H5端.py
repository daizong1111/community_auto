...
class TestProcess居民到物管(BaseCase):
    ...
    @pytest.mark.usefixtures("page_h5_一级网格员")
    @pytest.mark.usefixtures("page_h5_二级网格员")
    @pytest.mark.usefixtures("page_h5_三级网格员")
    @pytest.mark.usefixtures("page_h5_居民")
    @allure.step(
        "居民上报给社区，三级申请二级的协助，二级申请一级的协助，一级下发给二级，二级下发给三级，三级转派给物管，物管办结")
    def test_process_路径6(self, page_h5_居民, page_h5_三级网格员, page_h5_二级网格员,page_h5_一级网格员,
        ...
        角色列表 = [
            ...
            {"页面": page_h5_一级网格员, "表单数据": 表单数据_一级, "角色名称": "一级网格员"},
...
class TestProcess居民到一级(BaseCase):
    ...
    def test_process_路径1(self, page_h5_居民, page_h5_三级网格员, page_h5_二级网格员, page_h5_一级网格员,
                           表单数据_居民: dict,
    ...
    def test_process_路径2(self, page_h5_居民, page_h5_三级网格员,page_h5_一级网格员,
                           page_h5_二级网格员,
                           表单数据_居民: dict, 表单数据_物管: dict, 表单数据_物工: dict, 表单数据_三级: dict,
