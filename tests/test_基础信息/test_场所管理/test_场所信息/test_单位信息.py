...
def 单位信息页面(浏览器已打开的页面):
    # 将页面封装为单位信息页面
    page = PageUnitInfo(浏览器已打开的页面)
    yield page
...
class TestAdd(BaseCase):
    @pytest.mark.parametrize(
        ...
        [
            {
            # "单位名称": "新增-成功2025-06-26,17:17:41",
                 "单位名称": f"新增-成功{time_string}",
             "所属居委会": "中电数智街道/中电数智社区", "所属网格": "测试网格2",
             "统一信用代码": "91310115MA1K41MXQ5",
             "负责人": "石童涛", "联系方式": "13955499272", "单位类型": "测试单位",
    ...
    def test_add_success(self, 单位信息页面,
                         表单数据: dict
                         ):
        global NEW_PERSON_ADDED
        try:
            单位信息页面.click_button("新增")
            self.log_step("点击新增按钮")

            单位信息页面.快捷操作_填写表单_增加根据数据类确定唯一表单版(
                **表单数据)
            self.log_step("填写表单信息")

            单位信息页面.click_button("提交")
            单位信息页面.点击提示弹窗中的确定按钮()
            self.log_step("提交表单")

            expect(单位信息页面.page.get_by_text("新增成功")).to_be_visible(timeout=5000)
            self.log_step("验证新增成功-页面提示信息")

            负责人_加密后 = 表单数据.get("负责人")[0] + "*" * (len(表单数据.get("负责人")) - 1)
            单位信息页面.输入查询条件(单位名称=表单数据.get("单位名称"))
            单位信息页面.click_button("搜索")
            单位信息页面.等待表格加载完成()
            self.log_step("查询刚才新增的数据")
            loc_新增的行 = 单位信息页面.get_table_rows().filter(has_text=表单数据.get("单位名称")).filter(
    ...
    @pytest.mark.parametrize(
        ...
        [
            {"单位名称": f"彭超", "所属居委会": "中电数智街道/中电数智社区", "所属网格": "测试网格1213",
             "统一信用代码": "91310115MA1K41MXQ5",
             "负责人": "石童涛", "联系方式": "13955499272", "单位类型": "测试单位",
             "归属部门": "合肥市市场管理局", "入驻时间": "2015-07-13", "具体位置": "安徽省合肥市蜀山区芙蓉社区1005号",
             "执照日期": "2015-06-10,2029-07-28",
    ...
    def test_add_repeat_validation(self, 单位信息页面,
                                   表单数据: dict
                                   ):
        单位信息页面.输入查询条件(单位名称=表单数据.get("单位名称"))
        单位信息页面.click_button("搜索")
        新增前的数据量 = 单位信息页面.获取页面统计的总数据量()
        # 点击新增按钮
        单位信息页面.click_button("新增")
        self.log_step("点击新增按钮")
        # 填写表单信息
        单位信息页面.快捷操作_填写表单_增加根据数据类确定唯一表单版(**表单数据)
        self.log_step("填写表单信息")
        # 点击提交按钮
        单位信息页面.click_button("提交")
        单位信息页面.点击提示弹窗中的确定按钮()
        ...
        单位信息页面.输入查询条件(单位名称=表单数据.get("单位名称"))
        单位信息页面.click_button("搜索")
        新增后的数据量 = 单位信息页面.获取页面统计的总数据量()
        # 检查表格中是否有新增的数据
        assert 新增前的数据量 == 新增后的数据量
        self.log_step("验证查询列表中无新增的数据")


@pytest.mark.usefixtures("单位信息页面")  # 显式声明夹具
class TestEdit(BaseCase):
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                    {
                        "单位名称": f"新增-成功{time_string}",
                        # "单位名称": "新增-成功2025-06-26,16:46:16"
                    },
                    {"单位名称": f"修改-成功{time_string}", "所属居委会": "中电数智街道/中电数智社区",
                     "所属网格": "788",
                     "统一信用代码": "91440101MA5JL12345",
                     "负责人": "力洋", "联系方式": "18855429112",
    ...
    @allure.step("测试修改小区")
    def test_edit_success(self, 单位信息页面,
        ...
        单位信息页面.点击提示弹窗中的确定按钮()
        # 断言操作成功字样在页面出现
        expect(单位信息页面.page.get_by_text("编辑成功")).to_be_visible(timeout=5000)
        ...
        单位信息页面.输入查询条件(**{"单位名称": f"修改-成功{time_string}"})
        单位信息页面.click_button("搜索")
        单位信息页面.等待表格加载完成()
        # 检查表格中数据是否成功修改
        单位信息页面.点击表格中某行按钮(行号=1, 按钮名="编辑")
        单位信息页面.校验表单中数据成功修改(单位名称=表单数据.get("单位名称"),
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                    {
                        "单位名称": f"修改-成功{time_string}",
                        # "单位名称": "新增-成功2025-06-26,11:46:24"
                    },
                    {"单位名称": f"彭超", "所属居委会": "中电数智街道/中电数智社区",
                     "所属网格": "测试网格1213",
                     "统一信用代码": "91440101MA5JL12345",
                     "负责人": "力洋", "联系方式": "18855429112",
                     "归属部门": "淮南市人力资源局", "入驻时间": "2025-02-13",
    ...
    def test_edit_repeat_validation(self, 单位信息页面, 表单数据_搜索框: dict,
                                    表单数据: dict
                                    ):
        # 输入查询条件
        单位信息页面.输入查询条件(**表单数据_搜索框)
        ...
        单位信息页面.等待表格加载完成()
        # 检查原来的数据是否没了
        expect(单位信息页面.get_table_rows()).to_have_count(1)
...
class TestDetail(BaseCase):
    ...
    @allure.step("测试商铺详情")
    def test_detail(self, 单位信息页面, 行号):
        list_某行 = 单位信息页面.获取表格中指定行的所有字段值(行号)
        单位信息页面.点击表格中某行按钮(行号=行号, 按钮名="详情")
        单位信息页面.校验表单中数据成功修改(
            **{"单位名称": list_某行[2], "所属居委会": list_某行[4],
    ...
    class TestQuery(BaseCase):
        ...
        @pytest.mark.parametrize(
            ...
            [
                ...
                {"单位名称": "餐饮"},
                {"商铺类型": "餐饮用气场所"},
                {"区域": "中电数智街道/中电数智社区", "所属网格": "测试网格1213", "单位名称": "餐饮",
                 "商铺类型": "餐饮用气场所"},

            ]
        )
        def test_query(self, 单位信息页面, 表单数据: dict):
            # 输入查询条件
            单位信息页面.输入查询条件(**表单数据)
            单位信息页面.click_button("搜索")
            单位信息页面.等待表格加载完成()
            ...
            # 定义字段与验证逻辑的映射
            def verify_区域():
                列表_区域 = 单位信息页面.get_column_values_by_name("社区")
            ...
            def verify_所属网格():
                列表_所属网格 = 单位信息页面.get_column_values_by_name("所属网格")
                所属网格_预期值 = 表单数据["所属网格"]
            ...
            def verify_商铺名称():
                列表_商铺名称 = 单位信息页面.get_column_values_by_name("名称")
                商铺名称_预期值 = 表单数据["单位名称"]
                # 断言 列表_小区名称 中的每一项都包含 小区名称_预期值
                assert all(
                    商铺名称_预期值 in 单位名称 for 单位名称 in
                    列表_商铺名称), f"查询条件-单位名称:{商铺名称_预期值}, 表格中的商铺名称为:{列表_商铺名称}"

            def verify_商铺类型():
                列表_商铺类型 = 单位信息页面.get_column_values_by_name("类型")
                商铺类型_预期值 = 表单数据["商铺类型"]
                # 断言 列表_小区名称 中的每一项都包含 小区名称_预期值
                assert all(
...
class TestReset(BaseCase):
    def test_reset(self, 单位信息页面):
        ...
        # 点击重置按钮，获取查询接口响应的数据
        单位信息页面.click_button("重置")

        单位信息页面.校验查询条件成功修改(**{"区域": "", "所属网格": "", "归属部门": "",
               "单位名称": "", "商铺类型": "",})
...
class TestDelete(BaseCase):
    ...
    #     ]
    # )
    # def test_delete_success(self, 单位信息页面, 表单数据_查询, 删除原因):
    #     # 查找待删除的记录
    #     # 输入查询条件
    #     单位信息页面.输入查询条件(**表单数据_查询)
    #     单位信息页面.click_button("搜索")
    #     单位信息页面.等待表格加载完成()
    #     self.log_step("输入查询条件")
    #     删除前的数据量 = 单位信息页面.获取页面统计的总数据量()
    #     # 点击删除按钮
    #     单位信息页面.点击表格中某行按钮(行号=1, 按钮名="删除")
    #     单位信息页面.填写输入框_删除原因(删除原因)
    #     单位信息页面.click_button("确定")
    #     单位信息页面.点击提示弹窗中的确定按钮()
    #     self.log_step("点击删除按钮,弹窗后输入删除原因，再点击确定按钮")
    #     expect(单位信息页面.page.get_by_text("提交成功")).to_be_visible(timeout=5000)
    #     self.log_step("验证页面出现删除成功字样")
    #     # 跳转到审批页面，进行审批
    #
    #     单位信息页面.快捷操作_填写表单_增加根据数据类确定唯一表单版(**表单数据_查询)
    #     单位信息页面.click_button("搜索")
    #     单位信息页面.等待表格加载完成()
    #     删除后的数据量 = 单位信息页面.获取页面统计的总数据量()
    ...
    def test_delete_cancel(self, 单位信息页面, 表单数据_查询):
        # 输入查询条件
        单位信息页面.输入查询条件(**表单数据_查询)
        单位信息页面.click_button("搜索")
        单位信息页面.等待表格加载完成()
        ...
        # 点击删除按钮
        单位信息页面.点击表格中某行按钮(行号=1, 按钮名="删除")
        单位信息页面.click_button("关闭")
        self.log_step("点击删除按钮,弹窗后点击取消按钮")
        expect(单位信息页面.page.get_by_text("提交成功")).not_to_be_visible(timeout=5000)
