...
class TestDelete(BaseCase):
    ...
    def test_delete_fail(self, 商铺信息页面, 表单数据_查询, 删除原因, 审批不通过原因):
        # 查找待删除的记录
        # 输入查询条件
        ...
        # 商铺信息页面.page.wait_for_timeout(1000)
        ...
        # 跳转到审批页面，进行审批
        商铺信息页面.跳转到某菜单("基础信息", "场所管理/删除申请审批")
        删除申请审批页面 = PageDeleteApproval(商铺信息页面.page)
        ...
        商铺信息页面 = PageStoreInfo(删除申请审批页面.page)
        商铺信息页面.输入查询条件(**表单数据_查询)
        商铺信息页面.click_button("搜索")
        商铺信息页面.等待表格加载完成()
    ...
    def test_delete_success(self, 商铺信息页面, 表单数据_查询, 删除原因):
        ...
        商铺信息页面.点击表格中某行按钮(行号=1, 按钮名="删除")
        商铺信息页面.填写输入框_删除原因(删除原因)
        if 商铺信息页面.获取弹窗_删除申请().get_by_text("重新申请").count() > 0:
            商铺信息页面.获取弹窗_删除申请().get_by_text("重新申请").click()
        else:
            商铺信息页面.获取弹窗_删除申请().get_by_text("确定").click()
        商铺信息页面.点击提示弹窗中的确定按钮()
        self.log_step("点击删除按钮,弹窗后输入删除原因，再点击确定按钮")
        expect(商铺信息页面.page.get_by_text("提交成功")).to_be_visible(timeout=5000)
        self.log_step("验证页面出现删除成功字样")
        ...
        expect(删除申请审批页面.page.get_by_text("提交成功")).to_be_visible()
        # 跳转回商铺信息页面
        删除申请审批页面 = PageStoreInfo(商铺信息页面.page)
        删除申请审批页面.跳转到某菜单("基础信息", "场所管理/场所信息")
        商铺信息页面 = PageStoreInfo(删除申请审批页面.page)
        商铺信息页面.输入查询条件(**表单数据_查询)
        商铺信息页面.click_button("搜索")
