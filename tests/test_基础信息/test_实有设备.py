...
@pytest.mark.usefixtures("实有设备页面")
class TestAdd(BaseCase):
    ...
    def test_add_success(self, 实有设备页面,
                         表单数据: dict
                         ):
        global NEW_PERSON_ADDED
        try:
            实有设备页面.click_button("新增")
            self.log_step("点击新增按钮")

            实有设备页面.快捷操作_填写表单_增加根据数据类确定唯一表单版(**表单数据)
            # 实有设备页面.快捷操作_填写表单_增加根据数据类确定唯一表单版(籍贯="北京市/市辖区/东城区",)
            ...
            expect(实有设备页面.page.get_by_text("添加成功")).to_be_visible(timeout=5000)
            self.log_step("验证新增成功-页面提示信息")

            实有设备页面.填写搜索框({"选择小区": 表单数据.get("小区"), "车牌号": 表单数据.get("车牌号码")})

            小区_待寻找 = 表单数据.get("小区").split("/")[-1]
            车牌号_待寻找 = 表单数据.get("车牌号码")[0:3] + "***" + 表单数据.get("车牌号码")[-1]
            联系电话_待寻找 = 表单数据.get("车主联系方式")[0:3] + "****" + 表单数据.get("车主联系方式")[-4:]

            loc_新增的行 = 实有设备页面.get_table_rows().filter(has_text=小区_待寻找).filter(
    ...
    def test_add_repeat_validation(self, 实有设备页面,
                                   表单数据: dict
                                   ):
        # 点击新增按钮
        实有设备页面.click_button("新增")
        ...
        实有设备页面.填写搜索框({"选择小区": 表单数据.get("小区"), "车牌号": 表单数据.get("车牌号码")})
        # 检查表格中是否有新增的数据
        expect(实有设备页面.get_table_rows()).to_have_count(1)
...
class TestEdit(BaseCase):
    ...
    def test_edit_success(self, 实有设备页面,
        ...
        # 实有设备页面.点击提示弹窗中的确定按钮()
        # 断言操作成功字样在页面出现
        expect(实有设备页面.page.get_by_text("编辑成功")).to_be_visible(timeout=5000)
        self.log_step("验证修改成功-页面提示信息")
        实有设备页面.填写搜索框(表单数据_搜索框)
        # 检查表格中数据是否成功修改
        实有设备页面.点击表格中的按钮(None, "查看")
        实有设备页面.校验表单中数据成功修改(车主姓名=表单数据.get("车主姓名")[0] + "*"*len(表单数据.get("车主姓名")),
        ...
        实有设备页面.校验表单中时间成功修改(表单数据.get("时间"))
...
class TestQuery(BaseCase):
    ...
    def test_query(self, 实有设备页面, 表单数据: dict):
        ...
        def verify_设备名称或编码():
            ...
            预期值 = 表单数据["设备名称或编码"]
            # 断言 列表_小区名称 中的每一项都包含 小区名称_预期值
            assert all(预期值 in 设备名称 for 设备名称 in 列表_设备名称) or all(预期值 in 设备编码 for 设备编码 in 列表_设备编码), f"查询条件-设备名称或编码:{预期值}, 表格中的设备名称为:{列表_设备名称}，表格中的设备编码为:{列表_设备编码}"
...
class TestReset(BaseCase):
    def test_reset(self, 实有设备页面):
        # 输入查询条件
        实有设备页面.输入查询条件(选择小区="中电数智街道/中电数智社区/小区99", 设备名称或编码="666", 设备状态="在线", 使用场景="重点人员抓拍")
