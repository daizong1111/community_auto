...
def 新增数据_批量删除前(房屋管理页面, db_connection, 随机门牌号, 新增的数据量):
    test_add = TestAdd()
    test_add.test_add_success(房屋管理页面, db_connection, 随机门牌号, "天王巷小区", "官塘新村", "1单元", "102", "自住",
             "1层", "住宅", "国有房产", "100",
...
class TestDeleteBatch(BaseCase):
    def test_delete_batch_success(self, 房屋管理页面, db_connection, 待删除数据的门牌号, 删除数据数量):
        # 待删除数据的门牌号 = "103"
        # 从数据库中统计状态为删除的数据条数
        数据库中数据量_删除前 = 房屋管理页面.统计数据库表中的记录数(db_connection)
        # 查找待删除的记录
        房屋管理页面.快捷操作_填写表单_增加根据数据类确定唯一表单版(门牌号=待删除数据的门牌号)
        查询接口返回的数据 = 房屋管理页面.获取查询接口响应的数据("搜索")
        查询接口数据量_删除前 = 查询接口返回的数据["data"]["total"]
        self.log_step("获取查询接口响应的数据量")
        # 勾选并点击批量删除按钮
        房屋管理页面.点击批量删除按钮(待删除数据的门牌号, 删除数据数量)
        房屋管理页面.click_button("确定")
